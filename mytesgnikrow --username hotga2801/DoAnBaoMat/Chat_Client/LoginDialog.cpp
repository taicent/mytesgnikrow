...
void LoginDialog::OnLogin() 
{
	if (doCheckData() == TRUE)// kiem tra nhap du lieu do dung khong
	{
		...
		{
			// client dung password lam key va thuat toan RC2 ma hoa password
			// server dua vao username , lay password tuong ung lam key giai ma bang RC2 
			// neu password giai ma ra duoc la password lay lam key thi ok 
			CString message;
			message="LOGIN RC2 "+m_UserAcount + " "; // LOGIN user passwd
			// chuyen m_Passwd tu CString sang PCHAR
			PCHAR cstr = (char*)(LPCTSTR)m_Passwd;

			Crytography encrypt("RC2" , cstr);// dung thuat toan RC2 va password lam key
			CString passwdEncrypted = encrypt.outputString(m_Passwd,ENCRYPT);// mahoa password
			message = message + passwdEncrypted;
			sendText(m_socketClient,message);// gui message di
			// server phai tach lay passwd va giai ma bang RC2 
			// sau do gui tra ve LOGIN [1,0]
...
void LoginDialog::SockMsg(WPARAM wParam, LPARAM lParam)
{
	...
	case FD_READ:
		...
		else
		{
			...
			if(strncmp(buffer,"LOGIN 1",7)== 0)
			{
					this->EndDialog(IDLOGIN);
					// hien dialog chat len
					
			}
			else
			if(strncmp(buffer,"LOGIN 0",7)== 0)
			{
...
void LoginDialog::OnBnClickedCreateacc()
{
	...
	if (doConnect() == TRUE)// khoi tao ket noi den server
		{
			CreateUserDlg Dlg = new CreateUserDlg(m_socketClient);
			Dlg.DoModal();			
		}	
