...
float FastForwardPass(FastForwardStruct *net)
{
                                            ...
                                            unit++)
  {
    float tot=0.0;
    FastForwardConnection *conn=unit->connections;
    int num=(unit->numInputs+7) / 8;
    switch (unit->numInputs & 7) {
      case 0: goto mod0;
      case 1: goto mod1;
      case 2: goto mod2;
      case 3: goto mod3;
    ...
    for (; num>0; num--) {
      mod0:
      tot+=conn->weight*conn->from->activation; conn++;
      mod7:
      tot+=conn->weight*conn->from->activation; conn++;
      mod6:
      tot+=conn->weight*conn->from->activation; conn++;
                         ...
                         char *netname, int max)
{
  ...
  for (i=net->firstOutput; i<net->numUnits; i++)
    fprintf(outf,"  long a%d;\n",i);
  for (i=net->firstHidden; i<net->numUnits; i++)
  {
    FastForwardUnit *unit=net->unitList+i;
    long offset;
    fprintf(outf,"  a%d=",i);
                    ...
                    double rate, double momentum, double decay)
{
  ...
  for (unit=net->unitList+net->firstHidden;
  ...
  for (unit=net->unitList+net->firstOutput; unit<net->unitList+net->numUnits;
                                            unit++) {
    for (int p=0; p<net->patterns; p++)
  ...
  for (unit=net->unitList+net->numUnits-1;
...
{
  FastForwardUnit *unit;
  int min=(net->patterns*id)/processors;
  int max=(net->patterns*(id+1))/processors;
  for (unit=net->unitList+net->firstHidden;
       unit<net->unitList+net->numUnits; 
       unit++)
  {
  ...
  for (unit=net->unitList+net->firstHidden;
    ...
    for (int p=min; p<max; p++)
      unit->train->delta[p]=0.0;
  for (unit=net->unitList+net->firstOutput;
       unit<net->unitList+net->numUnits;
       unit++)
    for (int p=min; p<max; p++)
      unit->train->delta[p]-=unit->train->activations[p];
  for (unit=net->unitList+net->numUnits-1;
    ...
    switch (unit->type) {
      case TanhUnitType: {
        ...
        FastForwardConnection *conn=unit->connections;
        FastConnectionTrain *connTrain=unit->train->conns;
        for (int c=0; c<unit->numInputs; c++)
        {
          FastForwardUnit *from=conn->from;
          float delta=0;
          ...
          connTrain->partialDelta[id]=delta;
          conn++;
          connTrain++;
        }
        break;
      ...
      case TanhUnitType: {
        ...
        for (int c=0; c<unit->numInputs; c++)
        {
          ...
          for (int p=0; p<processors; p++) 
            delta+=unit->train->conns[c].partialDelta[p];
          conn->weight+=
            thisRate*(delta+momentum*unit->train->conns[c].prevDelta);
          unit->train->conns[c].prevDelta=delta;
          conn->weight-=conn->weight*decay;
...
#ifndef NoHEBP
FastForwardStruct *FastConstructForward(network *net)
{
  FastForwardStruct *fastnet=new FastForwardStruct;
  fastnet->numUnits=net->n_units;
  fastnet->patterns=0;
  fastnet->trainBuffer=NULL;
  fastnet->unitTrain=NULL;
  fastnet->connTrain=NULL;
  fastnet->firstInput=1;
  fastnet->firstHidden=net->n_inputs;
  fastnet->firstOutput=net->n_inputs+net->n_hiddens;
  fastnet->unitList=new FastForwardUnit[fastnet->numUnits];
  fastnet->numConns=net->n_conns;
  fastnet->connections=new FastForwardConnection[net->n_conns];
  fastnet->connList=new FastForwardConnectionRef[net->n_conns];
  FastForwardConnection *conn=fastnet->connections;
  for (int i=0; i<fastnet->numUnits; i++)
  {
    ...
    if (i==0) fastnet->unitList[i].type=BiasUnitType; else
      if (i<net->n_inputs) fastnet->unitList[i].type=InputUnitType; else
        fastnet->unitList[i].type=TanhUnitType;
    if (net->unit[i].num_inputs==0) 
      fastnet->unitList[i].connections=NULL; else
        fastnet->unitList[i].connections=conn;
    conn+=net->unit[i].num_inputs;
  }
  for (i=0; i<net->n_conns; i++)
  {
    int from=net->conn[i].from-net->unit;
    int to=net->conn[i].to-net->unit;
    fastnet->unitList[to].connections[fastnet->unitList[to].numInputs].from=
      fastnet->unitList+from;
    fastnet->unitList[to].connections[fastnet->unitList[to].numInputs].
      weight=net->conn[i].weight;
    fastnet->connList[i].from=from;
    fastnet->connList[i].to=to;
    fastnet->connList[i].conn=
      &(fastnet->unitList[to].connections[fastnet->unitList[to].numInputs]);
...
{
  for (int i=0; i<fastnet->numConns; i++) {
    double max=1.0/sqrt(fastnet->unitList[fastnet->connList[i].to].numInputs);
    fastnet->connList[i].conn->weight=
...
void FastWriteNet(FastForwardStruct *net, FILE *outf)
{
  ...
  for (i=0; i<net->firstOutput-net->firstHidden; i++)
    fprintf(outf,"0 0 0 0\n");
  fprintf(outf,"\n%d outputs\n",net->numUnits-net->firstOutput);
  for (i=0; i<net->numUnits-net->firstOutput; i++)
    fprintf(outf,"0 0 0 0\n");
  fprintf(outf,"\n%d conns\n",net->numConns);
  for (i=0; i<net->numConns; i++) fprintf(outf,"%d %d -1\n",
                                          net->connList[i].from,
                                          net->connList[i].to);
}
