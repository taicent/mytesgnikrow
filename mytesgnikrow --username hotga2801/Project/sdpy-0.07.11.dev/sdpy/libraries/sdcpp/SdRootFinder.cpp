...
loop:
        ...
        if (err > 0.01) {
            cout << "Excessive error!" << endl;
            cout << "Enter new trial 'r': ";
            cin >> quad[1];
            cout << "Enter new trial 's': ";
            cin >> quad[0];
        }
        if (err > 1) goto loop;
        x[m-2] = quad[1];
...
// or returns -2 if all but the last coefficient are 0
// or returns -1 if all coefficients are 0
int sdFindRoots( double *a, int n, double *wr, double *wi )
{
	while(n>0) if(a[0] == 0) { ++a; --n; }
	if(!n) return a[0] != 0? -2: -1;
	int num = 0, numr;
	while(a[n] == 0)
	{
		*(wr++) = *(wi++) = 0;
