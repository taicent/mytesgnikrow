#-------------------------------------------------------------------------------
# Name:        detect
# Purpose:     Detect faces based on Haar features
#
# Author:      minhtri
#
# Created:     06/05/2007
...
from .. import fromIplImage, THaarClassifier, integral
...
class FaceDetector:
    ...
    def save_to_file(self,filepath):
        if not self.has_BinaryClassifier:
            raise SyntaxError('The face detector was created without a BinaryClassifier')
    ...
    def _load_from_file(self,filepath):
        if not self.has_textfile:
            raise SyntaxError('The face detector was created without using a cascade text file')
    ...
    def setup_resolution(self, width, height, scale_factor):
        """Prepare the face detector for fast face detection.
    ...
    def __init__(self, face_type, flag = 0, param1 = None, param2 = None):
        """Initialize an FaceDetector.
        
        Input:
            face_type: the type of face to detect,
                in ('frontal_face','profile_face','upper_body','lower_body','full_body')
            flag: the type of operation of the face detector
            ...
            param1: [optional] a strong classifier to classify a patch into face/non-face                
        ...
        if self.has_BinaryClassifier: # load the classifier from BinaryClassifier
            ...
            self.ivecs2 = zeros(self.ivecs.shape, 'int')
        if self.use_opencv:
            self.haar_cascade = cvLoadCast(haar_cascades[face_type],CvHaarClassifierCascade)
    ...
    def detect(self, img, scale_factor = 1.1, min_neighbors = 3):
        """Detect faces from a grayscale image.
        
        Input:
            img: a POINTER(IplImage) of depth IPL_DEPTH_8U and 1 or 3 channels
            scale_factor: The factor by which the search window is scaled 
                between the subsequent scans, for example, 1.1 means increasing window by 10%.
            min_neighbors: Minimum number (minus 1) of neighbor rectangles that makes up a face. 
        ...
        Output:
            z: an 'int' numpy.array of tuples (x,y,w,h) representing the face locations
        """
