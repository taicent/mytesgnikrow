...
int main(int argc, char *argv[])
{
  ...
  unsigned char *buf;
  if (argc!=2) {
    fprintf(stderr,"xaviercolor_test filename.ppm\n");
    exit(1);
  }
  SetDataPath(argv[0]);
  ...
  numFaces=Xavier_FindAllFacesColor(width,height,buf,&locArray);
  ...
  fprintf(stderr,"\nFinding first face in image...\n");
  if (Xavier_FindOneFaceColor(width,height,buf,
                         0,width,   /* Range of X coords for center of face */
                         0,height,  /* Range of Y coords for center of face */
                         20,height, /* Range of face sizes */
