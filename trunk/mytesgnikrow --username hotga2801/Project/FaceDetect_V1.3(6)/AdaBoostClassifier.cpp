...
void AdaBoostClassifier::TrainAdaBoost(int rounds,const bool update,const int method)
{
	...
	for(i=0;i<totalfeatures;i++) FillTheTable(table[i],classifiers[i]);
 	InitializeWeights();
...
void AdaBoostClassifier::AddOneSimpleClassifier(bool* used)
{
	...
	NormalizeWeight();
	TRACE("%f %f %f %f\n",*min_element(weights,weights+facecount)*10000,*max_element(weights,weights+facecount)*10000,
		     *min_element(weights+facecount,weights+totalcount)*10000,*max_element(weights+facecount,weights+totalcount)*10000);
