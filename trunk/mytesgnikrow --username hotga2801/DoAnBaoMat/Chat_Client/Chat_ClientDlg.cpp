...
BEGIN_MESSAGE_MAP(CChat_ClientDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTONLogout, &CChat_ClientDlg::OnBnClickedButtonlogout)
	ON_BN_CLICKED(IDC_BUTTONEditPasswd, &CChat_ClientDlg::OnBnClickedButtoneditpasswd)
END_MESSAGE_MAP()
...
void CChat_ClientDlg::OnSendMessage() 
{
	UpdateData();
	CString message;
	message +="MESSAGE "+ m_AccountInfo.m_StrUserName+ " : "+ m_CurrentMessage;
...
LRESULT CChat_ClientDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if (message == WM_CREATE)
	{
		...
		if (result == IDLOGIN)
		{
			...
			this->m_AccountInfo.m_StrUserName = loginDlg.m_UserAcount;
			this->m_AccountInfo.m_StrPassword = loginDlg.m_Passwd;
	...
	case FD_READ:
		{
			...
			else
			{
				...
				if(strncmp(buffer,"MESSAGE",7) == 0) // MESSAGE FromUser Mesg
				{
					m_AllMessage +=&buffer[6];
					m_AllMessage+="\r\n";
				}
				if(strncmp(buffer,"LISTUSER",8) == 0) // LISTUSER u1 u2 u3 ..
				{
					CString temp;
					temp = &buffer[6];
					if (temp!=m_AccountInfo.m_StrUserName)
					{
						m_listFriend.AddString (temp);
					}
					
				}
				if(strncmp(buffer,"LOGOUTMEMB",10) == 0)//LOGOUTMEMB username
				{
					CString userLogut;
					userLogut = &buffer[6];
				...
				if(strncmp(buffer,"ENCRYPTION",10) == 0) // ENCRYPTION XYZ
				{
					//_typeOfCrytography = XYZ;
				}
				if(strncmp(buffer,"NEWMEMB",7) == 0)// NEWMEMB username
				{
					// them username vao list
				}
				if(strncmp(buffer,"USERINFO",8) == 0)// USERINFO typeOfUser fullname description 
				{
					// cap nhat lai cho bien m_AccountInfo
				}
				if(strncmp(buffer,"UPDATE",6) == 0)// UPDATE [1,0]
				{
					// thong bao cap nhat thanh cong
				}
				if(strncmp(buffer,"EDITPASS",8) == 0)//EDITPASS[1,0]
				{
...
void CChat_ClientDlg::OnBnClickedButtonlogout()
{
	// TODO: Add your control notification handler code here
	// dong ket noi 
...
void CChat_ClientDlg::OnBnClickedButtoneditpasswd()
{
	// TODO: Add your control notification handler code here
